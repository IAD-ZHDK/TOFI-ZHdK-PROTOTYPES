...
class Meta {
  ...
  update (p, sensorValues, params) {
    ...
    for (let i = 0; i < this.blobs.length; i++) {
      this.blobs[i].Xamp = p.map(sensorValues[4], 30000, 33000, 2, this.width * 0.65)
      this.blobs[i].Yamp = p.map(sensorValues[5], 30000, 33000, 2, this.height * 0.65)
      this.blobs[i].r = 30 + p.map(sensorValues[6], 30000, 31000, 0, 100)
      this.blobs[i].speed = 0.01
